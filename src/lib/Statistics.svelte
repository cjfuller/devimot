<script lang="ts">
    ...
    $: toCopy = [`Devimot #${currState.puzzleNumber}${windicator}`, stateIntoShareGrid(currState)].join("\n");
</script>

<Container title="Statistiques" onClose={onClose}>
    <div class="title">
        Distribution d'essais historiques
    </div>
    <div class="bargraph">
        {#each [1, 2, 3, 4, 5, 6] as i}
            <Bar index={i} highlight={counts[i] === maxCount} colors={colors} proportion={total > 0 ? counts[i] / total : 0} />
        {/each}
        <Bar index={"perte"} highlight={false} colors={colors} proportion={total > 0 ? counts[10] / total : 0} />
    </div>
    <div class="spacer" />
    <div class="rule" />
    <div class="title">
    ...
    <div class="spacer" />
        ...
        {/each}
        <div class="spacer"/>
        <button on:click={() => globalThis.navigator.clipboard.writeText(toCopy)}>
            Copier
        </button>
</Container>
